...
export function mountTeamField(root, options) {
  ...
  function playerMarker(name, group, preview = false) {
    ...
    return `<button type="button" class="fieldPlayer ${upper(group)?'fieldPlayer--orange':''} ${selected===name?'isSelected':''}" data-name="${esc(name)}" style="left:clamp(24px, ${p.x}%, calc(100% - 24px));top:clamp(20px, ${p.y}%, calc(100% - 20px))" aria-label="${esc(name)}, ${esc(role)}, ${esc(group.label)}${captain?', captain':''}" ${canMove(name)?'':'disabled'}>${portrait}${captain?'<em aria-hidden="true">C</em>':''}<span>${esc(name)}</span><small>${role}</small></button>`;
  }
  ...
  function draw() {
    ...
    const bodyScroll = root.querySelector('.fieldWorkspace__body')?.scrollTop || Number(root.dataset.fieldScroll || 0);
    ...
    root.innerHTML = `<div class="fieldPreviewHeader">
      ...
      <dialog class="fieldDialog" aria-label="Team assignment and positions"><div class="fieldWorkspace">
        <header class="fieldWorkspace__head"><strong>Team field</strong><span class="small">${editable ? 'Tap a player, then tap the pitch to move them. You can also drag.' : 'Saved positions'}</span><button class="btn gray tiny" data-back>Back</button>${options.onSave && editable?'<button class="btn good tiny" data-save>Save</button>':''}</header>
    ...
    const positionPicker = root.querySelector('[data-position]');
    if (positionPicker) positionPicker.onchange = () => {
    ...
    root.querySelectorAll('[data-name]').forEach(button => {
      ...
      button.onclick = () => {
        ...
        root.querySelectorAll('[data-name]').forEach(element => { if (element.dataset.name===name) element.focus({preventScroll:true}); });
      };
