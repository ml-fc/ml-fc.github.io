...
const availabilityList = readRowsByMatchId(SHEET_AVAIL, match.matchId);
return jsonOut({ ok: true, updated: true, availability: availabilityList });
        }
      }
...
invalidateMatches(match.seasonId, match.publicCode);
const availabilityList = readRowsByMatchId(SHEET_AVAIL, match.matchId);
return jsonOut({ ok: true, updated: false, availability: availabilityList });
    }
